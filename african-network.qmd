...
An example of a modularity-based method is the fast greedy algorithm [@NewmanPRE04], which starts with a subnetwork formed only by edges between highly connected nodes. Then, in each iteration the algorithm samples random edges that increase the level of modularity in the subnetwork and adds them.
